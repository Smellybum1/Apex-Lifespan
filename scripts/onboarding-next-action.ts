...
interface OnboardingNextActionArgs {
  envFilePath?: string;
...
async function main() {
  ...
  await withProcessEnv(env, async () => {
    ...
    } = await import("@/lib/supplement-onboarding-next-action");
    const { buildSupplementOnboardingQualityDashboard } = await import(
      "@/lib/supplement-onboarding-quality"
    );
    ...
    if (args.supplement && report.rows.length === 0) {
      throw new Error(`No supplement found for onboarding next action: ${args.supplement}`);
    }

    console.log(
      JSON.stringify(
        args.summary ? summarizeSupplementOnboardingNextActionReport(report) : report,
        null,
        2
      )
    );
...
function readArgs(args: string[]): OnboardingNextActionArgs {
  const parsed: OnboardingNextActionArgs = {
    summary: false
  };
  ...
  for (let index = 0; index < args.length; index += 1) {
    ...
    throw new Error(`Unknown onboarding next-action option: ${arg}`);
  }
