...
- `--candidate-review-packet` prints command hints, detail, accepted-reference matches, sibling context, curation-status/draft hints, duplicate hints when the identity repeats, focused review-flag hints when the candidate is flagged, and explicit accept-gate booleans.
