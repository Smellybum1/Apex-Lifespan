...
- `--candidates` defaults to pending rows, prints `packet="..."` and `referenceMatches="..."` hints plus query/job trace fields and compact `reviewFlags` plus a `flags="..."` drill-in when applicable, and can filter by source, region, external id, job, intervention, claim, or decision.
