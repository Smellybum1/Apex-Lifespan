...
- `--candidate-curation-handoff` reports accepted-candidate readiness for public source packets, not evidence quality, and non-empty rows print read-only packet/reference/sibling/status/draft hints, duplicate hints plus `duplicateCaution` when the PMID/NCT identity repeats, accepted-reference/review-note fields for reviewed rows, and compact `reviewFlags`, `flags="..."`, and `flagFocus="..."` drill-ins when applicable.
