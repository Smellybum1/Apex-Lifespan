...
Curation handoff rows print read-only packet, reference-match, sibling, curation-status, curation-draft hints, `nextWrite`/`writeReady` plus `writeReview` draft drill-ins, duplicate hints plus `duplicateCaution` when the PMID/NCT identity repeats, accepted-reference audit fields plus review-note fields for reviewed rows, and compact `reviewFlags`/`reviewCautions` plus `flags="..."` and `flagFocus="..."` drill-ins when applicable.
