...
Curation handoff rows print read-only packet, reference-match, sibling, curation-status, curation-draft hints, duplicate hints plus `duplicateCaution` when the PMID/NCT identity repeats, accepted-reference/review-note fields for reviewed rows, and compact `reviewFlags`/`reviewCautions` plus `flags="..."` and `flagFocus="..."` drill-ins when applicable.
