...
Refreshed on 2026-06-05 after public route write-handler boundary coverage. Verify local state with `git status -sb` and `git log -1 --oneline` before edits.
...
- Branch: `codex/queue-claim-sources`; current code commit before this handoff refresh is `2873655 Guard public routes against write handlers`.
