...
Refreshed on 2026-06-05 after noindex live-source responses. Verify local state with `git status -sb` and `git log -1 --oneline` before edits.
...
- Branch: `codex/queue-claim-sources`; current code commit before this handoff refresh is `df8a7be Mark live-source responses noindex`.
