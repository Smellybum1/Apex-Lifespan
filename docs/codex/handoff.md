...
Refreshed on 2026-06-05 after disabling upstream live-source fetch caching. Verify local state with `git status -sb` and `git log -1 --oneline` before edits.
...
- Branch: `codex/queue-claim-sources`; current code commit before this handoff refresh is `4f07ffc Disable live source upstream caching`.
