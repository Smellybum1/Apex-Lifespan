...
Refreshed on 2026-06-04 after adding accepted-reference readiness hints to source-candidate review packets. Verify local state with `git status -sb` and `git log -1 --oneline` before edits.
...
Current code validation for review-packet accepted-reference readiness hints:
