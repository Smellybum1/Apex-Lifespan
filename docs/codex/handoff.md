...
- Dashboard header includes an `Ask Codex` approval button that builds a read-only Codex review packet from the current dashboard state, can copy it locally, or can send it to a token-gated `127.0.0.1` sidecar after approval. The sidecar resumes the configured Codex thread in read-only sandbox mode with approvals disabled.
- Dashboard-to-Codex packet sends use `npm run codex:review-sidecar`, `APEX_CODEX_THREAD_ID`, and `APEX_CODEX_REVIEW_TOKEN`; they must remain outside public Next.js API routes and must not accept/reject source candidates or promote evidence.
...
Latest code validation for `4f195a8`:
- Startup/workflow docs and current state re-read.
- `npm install @openai/codex-sdk`
- `npm run test -- src/components/evidence-dashboard.test.tsx src/app/api/live-source-readonly-boundary.test.ts scripts/codex-review-sidecar.test.ts`
- `npm run test`
- `npm run lint`
- `npm run dev:stop`
- `npm audit`
