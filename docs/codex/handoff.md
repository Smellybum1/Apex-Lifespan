...
Refreshed on 2026-06-05 after no-store live-source responses. Verify local state with `git status -sb` and `git log -1 --oneline` before edits.
...
- Branch: `codex/queue-claim-sources`; current code commit before this handoff refresh is `782c3f7 Prevent live-source response caching`.
