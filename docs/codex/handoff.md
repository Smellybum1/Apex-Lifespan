...
Refreshed on 2026-06-04 after adding source-candidate reference-match hints. Verify local state with `git status -sb` and `git log -1 --oneline` before edits.
...
- Summary output is read-only and prints next-command hints for overview, duplicate scan, queued jobs, and curation handoff.
