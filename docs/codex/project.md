...
- Commands:
  ...
  - Local source-candidate ingestion: `npm run ingest:sources` (operator-only; runs queued PubMed/ClinicalTrials.gov jobs against configured PostgreSQL); `npm run ingest:sources -- --summary` prints read-only source-candidate workflow counts.
...
- Architecture/patterns:
  ...
  - `npm run ingest:sources` is a private/local operator command for the source-candidate job runner; default execution is bounded to one queued job and writes only to configured PostgreSQL. Its `--summary` mode is read-only and groups source-candidate backlog counts by source, region, decision, and review status.
