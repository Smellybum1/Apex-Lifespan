...
- Architecture/patterns:
  ...
  - `npm run ingest:sources` is bounded, private/local, and documented in `docs/codex/source-candidate-workflow.md`; read-only modes inspect jobs, candidates, reference matches, curation status, handoff, and summary, while review modes remain operator-only writes.
