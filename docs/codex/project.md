...
- Architecture/patterns:
  ...
  - `npm run ingest:sources` is a private/local operator command for the source-candidate job runner; default execution is bounded to one queued job and writes only to configured PostgreSQL. Queue options create missing PubMed/ClinicalTrials.gov jobs or report the existing source/query/region job without resetting completed work. Its `--jobs`, `--candidates`, and `--summary` modes are read-only; `--jobs` lists recent ingestion job ids/statuses/counts/errors, `--candidates` lists source-candidate titles/URLs/dedupe keys with triage scores and defaults to pending rows, `--candidate-decision accepted|rejected` filters reviewed candidate records, and `--summary` groups source-candidate backlog counts by source, region, decision, and review status. Its review modes are operator-only writes for pending candidates: `--accept-candidate` requires an existing curated reference id whose source and external id match the candidate and still does not auto-promote evidence cards; `--reject-candidate` records a human-reviewed rejection note when supplied.
